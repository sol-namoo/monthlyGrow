...
export const usePageData = (
    page: 'home' | 'loopDetail' | 'projectDetail' | 'newLoop' | 'area' | 'resource' | 'archive',
    options?: any,
) => {
    if (page === 'home') {
        ...
        const { data: loops, isLoading: loopsLoading, error: loopsError } = useQuery({
            queryKey: ['loops'],
            queryFn: () => getLoopsByUserId('user1'),
        }); // user1은 임시 유저입니다.
        return { projects, loops, isLoading: loopsLoading || projectsLoading, error: loopsError || projectsError };
    }

    if (page === 'loopDetail') {
        const { loopId } = options;
        const { data: loop, isLoading: loopLoading, error: loopError } = useQuery({
    ...
    if (page === 'projectDetail') {
        ...
        return { project, tasks, isLoading: projectLoading || tasksLoading, error: projectError || tasksError };
    }
    if (page === 'newLoop') {
