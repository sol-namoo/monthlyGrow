...
export const monthlyDetail = {
  ...
  retrospectiveTab: {
    keyResultsStatus: "핵심 지표 현황",
    completedKeyResults: "달성한 핵심 지표 ({completed}/{total})",
    failedKeyResults: "달성하지 못한 핵심 지표 ({failed}/{total})",
    keyResultProgress: "목표: {target}회 → 달성: {completed}회",
    noKeyResults: "설정된 핵심 지표가 없습니다.",
    stuckPoints: "막힌 지점",
    newLearnings: "새로운 학습",
    memorableTask: "가장 기억에 남는 작업",
    failedKeyResultsAnalysis: "핵심 지표 실패 이유 분석",
  },
  note: {
