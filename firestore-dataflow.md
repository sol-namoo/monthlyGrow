...
### 1. Project-Chapter 연결

- **단방향 관계**: Chapter의 `projectIds[]`로 Project 참조
- **챕터 생성 시**: 선택된 프로젝트들을 Chapter의 `projectIds[]`에 추가
- **프로젝트 생성 시**: Chapter ID를 Project의 `chapterId`에 저장 (legacy)
- **데이터 정합성**: Chapter에서 Project 목록 관리, 필요시 쿼리로 조인
...
4. 챕터 생성
5. 선택된 프로젝트들의 connectedChapters[] 업데이트
6. 챕터의 projectIds[] 업데이트
```
