...
export default function LoopDetailPage({ params }: { params: { id: string } }) {
  ...
  const loop = {
    ...
    areas: ["건강", "개발", "마음"],
    projects: [
      { id: 1, title: "아침 운동 습관화", progress: 25, total: 30, addedMidway: false },
      { id: 2, title: "식단 관리 앱 개발", progress: 10, total: 12, addedMidway: false },
      { id: 3, title: "명상 습관 만들기", progress: 15, total: 20, addedMidway: false },
      { id: 4, title: "건강 블로그 작성", progress: 2, total: 8, addedMidway: true },
    ],
    completed: false,
    reflection: null,
  }
      ...
      <Card className="mb-6 p-4">
        ...
        <div className="mb-4">
          ...
          <div className="progress-bar">
            <div className="progress-value" style={{ width: `${loop.progress}%` }}></div>
          </div>
        </div>
        ...
        <div className="mb-4">
          <div className="mb-2 flex items-center justify-between">
            ...
            {!loop.completed && (
              <Button variant="outline" size="sm" onClick={handleAddProject} disabled={!canAddProject}>
      ...
      <Dialog open={showAddProjectDialog} onOpenChange={setShowAddProjectDialog}>
        <DialogContent>
          ...
          <div className="flex flex-col gap-4 py-4">
            <Button asChild>
              <Link href="/project/new?loopId=1&addedMidway=true">새 프로젝트 생성</Link>
            </Button>
            <Button variant="outline" asChild>
              <Link href="/loop/add-existing-project?loopId=1">기존 프로젝트 연결</Link>
            </Button>
          </div>

          <DialogFooter>
            <Button variant="secondary" onClick={() => setShowAddProjectDialog(false)}>
              취소
            </Button>
          </DialogFooter>
        </DialogContent>
