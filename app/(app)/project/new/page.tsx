...
export default function NewProjectPage() {
  ...
  const isLoopFull = selectedLoop && selectedLoop.id !== "none" && selectedLoop.projectCount >= 5
    ...
    <div className="container max-w-md px-4 py-6">
      <div className="mb-6 flex items-center">
        <Button variant="ghost" size="icon" asChild className="mr-2">
          <Link href={loopId ? `/loop/${loopId}` : "/project"}>
      ...
      {addedMidway && currentLoop && (
        <Alert className="mb-6 bg-amber-50">
          ...
          <AlertTitle className="text-amber-600">루프 중간에 추가되는 프로젝트</AlertTitle>
          <AlertDescription className="text-amber-600">
            이 프로젝트는 <Badge className="bg-primary/20">{currentLoop.title}</Badge>에 중간에 추가되는 프로젝트로
            표시됩니다. 월말 리포트에서 '후속 투입 항목'으로 별도 집계됩니다.
          </AlertDescription>
        </Alert>
      )}
      ...
      {isLoopFull && (
        <Alert className="mb-6 bg-red-50">
          ...
          <AlertTitle className="text-red-600">루프 프로젝트 개수 초과</AlertTitle>
          <AlertDescription className="text-red-600">
            선택한 루프에는 이미 5개의 프로젝트가 있습니다. 다른 루프를 선택하거나 루프에 포함하지 않음을 선택하세요.
          </AlertDescription>
        </Alert>
      )}
        ...
        <Card className="mb-6 p-4">
          ...
          <div className="mb-4">
            ...
            >
              ...
              <SelectContent>
                {loops.map((loop) => (
                  <SelectItem key={loop.id} value={loop.id} disabled={loop.id !== "none" && loop.projectCount >= 5}>
                    {loop.name} {loop.id !== "none" && `(${loop.projectCount}/5)`}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            <p className="mt-1 text-xs text-muted-foreground">
              ...
              {loopId && " 루프 상세 페이지에서 생성 시 자동으로 해당 루프에 연결됩니다."}
            </p>
          </div>
